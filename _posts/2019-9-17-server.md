...
简单来说，<font color="#26975b"><b>服务器 ( Server )</b></font> 就是一台台日夜不休的电脑，它们通常被放在专用的机房里以提供一个稳定的计算环境。我们可以简单的认为服务器在结构组成上和**个人电脑 ( Personal Computer, PC )** 的主机几乎没有区别，该有的结构，像 CPU 、硬盘等它都有，但是由于需要提供高可靠的服务，因此在处理能力、稳定性、可靠性、安全性、可扩展性、可管理性等方面要求较高。当我们需要长时间稳定的高性能计算时，个人电脑可能会因为种种原因崩溃死机重启，这时服务器就是我们最好的选择。

对于初学者而言，唯一不同的地方就是服务器上运行的**操作系统 ( Operating System, OS )** 和 PC 上的 Windows 系统或者 MacOS 系统不同，服务器上一般运行的是各种发型版本的开源 Linux 系统（ 国内多使用 [**CentOS**](https://www.centos.org) ）或者微软公司推出的 [**Windows Server**](https://www.microsoft.com/zh-cn/cloud-platform/windows-server) 系统，后者相对比较少见。我们主要介绍使用 Linux 系统的服务器，本文默认大家都掌握一些最基本的 Linux 命令。

## **远程登录服务器**

<hr>

前面说到服务器就是很多放在机房里的电脑主机，所以当我们需要使用它们的时候，就需要通过远程登录进而控制服务器进行计算，一般我们使用 [**SSH**](https://www.ssh.com) 来远程登录服务器。
...
登录后可以看到服务器管理员写的一些使用须知，熟悉 Linux 系统的同学应该立刻就发现了：现在我们就在服务器系统当前登录账户的**根目录 ( ~ )** 下了，剩下的一切就和 Linux 系统相同了。
...
* 有时我们会发现 SSH 连接莫名其妙的断开了，其实这是因为你一段时间没有任何操作后 SSH 自动断开，我们只需要进入本机根目录下的 .ssh 文件夹，在 **config** ( 如果没有就自己使用 **Vim** 或其他**文本编辑器**新建一个 ) 文件中添上一句：
``` sh
ServerAliveInterval 60
```
就可以解决这个问题了。

* 有时**本机用户名**和我们**登录服务器所用的用户名**不同，我们可以在 cofig 文件中写上
``` sh
Host address
	User usrname
```
同样 `address` 是服务器在内网中的地址，`usrname` 是你**登录服务器所用的用户名**。保存后我们直接使用 `ssh address` 命令就可以登录服务器了。
...
```
* 如果传输的是**文件夹**，则在 `scp` 后加上 `-r` 表示**递归 ( recurse )** 就好了:
``` sh
...
将其复制到远程服务器的 **~/.ssh** 目录下，并命名为 **authorized_keys** 就 OK 了，以后若有别的秘钥，直接添加到 authorized_keys 文件末尾就好了。
...
对于**单进程串行**程序，其编译执行过程同 Linux 系统中一样。在服务器上我们可以通过 `nohup ... &` 命令来<font color="#26975b"><b>守护进程</b></font>，**守护进程**的意思就是即便我们退出登录，服务器仍会继续运行程序。比如使用 `nohup command > output 2>&1 &` 命令，其中 `command` 为所执行任务的命令，输出结果到 output 文件，`2>&1` 表示把**标准错误** ( 2 ) 重定向到**标准输出** ( 1 ) 中，这样一来如果报错的话，**错误信息**也会被输出到 output 文件中。我们可以通过 `jobs`, `ps` 或者 `ps -ef` 命令来查看任务信息（ `ps -ef | grep usrname ` 可以查看指定用户正在执行的任务 ），使用 `kill PID` 命令来杀死**进程 ( Process )** ，其中 `PID` 为任务信息提供的**进程号**。
